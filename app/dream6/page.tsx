...
export default function Dream9Page() {
  // DREAM9 FIXED: 9 slots, front preview, responsive 2-column desktop layout
  ...
  function chooseEnthusiastCategory(category: EnthusiastCategory) {
    const selectedCars = ENTHUSIAST_CATEGORIES[category].slice(0, 9).map((model) => {
    ...
    const missingModels = ENTHUSIAST_CATEGORIES[category].slice(0, 9).filter(
  ...
  async function shareDream9() {
    ...
    try {
      ...
      console.error("SHARE DREAM 3 FAILED:", error);
  ...
  async function makePoster(size: ShirtSize) {
    ...
    try {
      ...
      const checkoutUrl =
        ...
        `&quantity=1` +
        `&properties[Design Type]=${encodeURIComponent("Dream 9")}` +
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-black px-4 py-5 text-white md:p-6">
      ...
      <div className="pointer-events-none fixed left-0 top-0 opacity-0">
        ...
        >
          <Dream9ExportDesign exportMode title="Dream 9" />
        </div>
        ...
        >
          <Dream9ExportDesign exportMode title="My Dream 9" />
        </div>
      </div>
    </main>
  );
